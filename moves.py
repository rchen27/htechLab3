...
class Moves():
	...
	def auth(self, request_token):
		...
		j = requests.post('https://api.moves-app.com/oauth/v1/access_token?grant_type=authorization_code&code=' + request_token + c + s + r)
	...
	def get_profile(self, token):
		token = '?access_token=' + token
		root = '/user/profile'
		return requests.get('https://api.moves-app.com/api/1.1' + root + token).json()
	...
	def get_summary(self, token, date):
		token = '?access_token=' + token
		return requests.get('https://api.moves-app.com/api/1.1' + '/user/summary/daily/' + date + token).json()

	#for htech
	def get_locations(self, token, date):
		token = '?access_token=' + token
		return requests.get('https://api.moves-app.com/api/1.1' + '/user/places/daily/' + date + token).json()
